...
class UnifiedParser:
    ...
    def __init__(self):
        ...
        # Only process Java and Python files
        # We'll use tree-sitter for java and python, and skip all other files
    ...
    def parse_java_file(self, file_path: str, repo_path: str) -> List[CodeUnit]:
        ...
        """
        # First try to use tree-sitter for Java parsing if available
        if 'java' in self.tree_sitter_parser.languages:
            return self.tree_sitter_parser.parse_file(file_path, repo_path, self.stats)

        # Fall back to javalang if tree-sitter is not available for Java
        code_units = []
        ...
        # Now try to parse the Java content with javalang
        try:
            tree = javalang.parse.parse(content)
            relative_path = os.path.relpath(file_path, repo_path)
            code_units.extend(self._extract_java_code_units(tree, relative_path, content))
        except Exception as e:
            error_msg = f"Error parsing Java file {file_path} with javalang: {str(e)}"
            print(error_msg)
